<script lang="ts">
  import toast from 'svelte-french-toast'
  import { createDialog } from 'svelte-headlessui'
  import { ArrowUturnLeft, ExclamationCircle, Icon } from 'svelte-hero-icons'
  import { superForm } from 'sveltekit-superforms/client'

  import { FormControl, FormMessage, Modal, PageLayout } from '$components'

  export let data

  const { form, enhance, message, errors, constraints, delayed } = superForm(
    data.form,
    {
      dataType: 'json',
      onResult: ({ result: { type } }) => {
        // show toast notification letting user know the update was successful before redirecting
        if (type === 'redirect') {
          toast.success('User updated!')
        }
      }
    }
  )

  const deleteModal = createDialog({ label: 'delete user' })
</script>

<svelte:head>
  <title>Edit User | VF Columbus</title>
</svelte:head>

<PageLayout contentContainerStyles="max-w-3xl">
  <svelte:fragment slot="header">
    <h1>Edit User</h1>
    <a
      href="/admin/users"
      class="link link-primary"
    >
      <Icon
        src={ArrowUturnLeft}
        size="1em"
      />
      All Users
    </a>
  </svelte:fragment>

  <button
    type="button"
    class="btn btn-danger mb-8"
    on:click={deleteModal.open}>Delete This User</button
  >

  <form
    method="post"
    action="?/updateUser"
    use:enhance
    class="form"
  >
    <p class="text-xl">
      Email: <span class="font-semibold">{data.profile.email}</span>
    </p>
    <FormControl
      label="Name"
      errors={$errors.name}
    >
      <input
        type="text"
        bind:value={$form.name}
        data-invalid={$errors.name}
        {...$constraints.name}
      />
    </FormControl>

    <FormControl
      label="Role"
      errors={$errors.role}
    >
      {#if data.profile.id === data.session?.user.id && data.profile.role === 'Admin'}
        <span class="flex items-center gap-1 text-sm text-warning-400">
          <Icon
            src={ExclamationCircle}
            size="1.5em"
            class="inline shrink-0"
          />
          If you change your own role to anything but Admin, only a different Admin
          user can change it back.
        </span>
      {/if}
      <select
        bind:value={$form.role}
        data-invalid={$errors.role}
        {...$constraints.role}
      >
        <option value={null}>None</option>
        <option value={'Store'}>Store</option>
        <option value={'Manager'}>Manager</option>
        <option value={'Admin'}>Admin</option>
      </select>
    </FormControl>

    <FormControl
      label="Locations"
      errors={$errors.locations}
    >
      <div class="flex flex-wrap gap-x-8 gap-y-2">
        {#each data.locations as location}
          <label class="checkbox">
            <input
              type="checkbox"
              bind:checked={$form.locations[location.id]}
            />
            <span>{location.name}</span>
          </label>
        {/each}
      </div>
    </FormControl>

    <FormMessage message={$message} />

    <button
      type="submit"
      disabled={$delayed}
      class="btn btn-primary">{$delayed ? 'Updating...' : 'Update User'}</button
    >
  </form>

  <Modal modalStore={deleteModal}>
    <p>Are you sure you want to delete this user?</p>
    <div class="mt-8 grid w-full grid-cols-2 gap-4">
      <form
        method="post"
        use:enhance
        action="?/deleteUser"
      >
        <button
          type="submit"
          class="btn btn-danger w-full">Yes</button
        >
      </form>
      <button
        type="button"
        class="btn"
        on:click={deleteModal.close}>No</button
      >
    </div>
  </Modal>
</PageLayout>
